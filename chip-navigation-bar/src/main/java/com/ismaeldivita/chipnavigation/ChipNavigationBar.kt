...
class ChipNavigationBar @JvmOverloads constructor(
    ...
    fun setItemEnabled(id: Int, isEnabled: Boolean,title:String) {
        getItemById(id)?.isEnabled = isEnabled
        getItemById(id)?.setTitle(title)
    }
    ...
    fun setItemSelected(id: Int, isSelected: Boolean = true,title: String) {
        setItemSelected(id = id, isSelected = isSelected, dispatchAction = true, title = title)
    }
    /**
    ...
    fun setItemSelected(id: Int, isSelected: Boolean, dispatchAction: Boolean,title: String) {
        val selectedItem = getSelectedItem()

        when {
            isSelected && selectedItem?.id != id -> {
                selectedItem?.isSelected = false
                getItemById(id,title)?.let {
                    beginAnimation()
                    it.isSelected = true
                    if (dispatchAction) {
                        listener?.onItemSelected(id)
    ...
    private fun getItemById(id: Int,title: String) = getChildren()
        .filterIsInstance<MenuItemView>()
        .firstOrNull {
            it.setTitle(title = title)
            it.id == id
        }
    /**
